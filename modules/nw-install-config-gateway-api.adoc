...
[NOTE]
====
Envoy is exposed using a LoadBalancer service. Replace `gateway.yaml` with `gateway-nodeport.yaml` to use a NodePort service instead.
====
+
.Verification
+
...
[NOTE]
====
The example above defaulted to pod `envoy-g86st` since the daemonset has three running pods. Use a different pod if the curl request does not appear in the logs.
====
